...
# a single RUN layer, so the layer diff carries no secret. Render masks
# env-var values in build logs.
...
RUN set -eu \
    ...
    && chmod +x /askpass.sh \
    && GIT_ASKPASS=/askpass.sh GIT_TERMINAL_PROMPT=0 git clone https://github.com/Fuhad-Lab/longhorizon-agent-platform src \
    && rm -f /askpass.sh \
